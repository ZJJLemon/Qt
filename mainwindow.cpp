...
void MainWindow::SerialInit() //串口初始化
{
    //设置数据位 8位
...
QList<int> MainWindow::getArrY()
{
    QList<int> ArrY;

    for(int j=0;j<45;j++)
    {
        ArrY.append(QRandomGenerator::global()->bounded(0,250));
    }

    return ArrY;
}

QList<int> MainWindow::getArrX()
{
    QList<int> ArrX;
    //生成0-44排序好的列表X
    for(int i=0;i<45;i++)
...
void MainWindow::timeReadData() // 定时接收数据
{
    if(ReceiveFlag==1)
    {
        serialPortReadyRead();
...
void MainWindow::on_BackHome_clicked()
{
    ui->stackedWidget->setCurrentIndex(0);
}
// /////////////////

void MainWindow::on_serialSelect_activated(int index)
{
...
void MainWindow::on_c1_toggled(bool checked)
{
    if(checked)
    {
        splineseries1->show();
...
void MainWindow::on_c2_toggled(bool checked)
{
    if(checked)
    {
        splineseries2->show();
...
void MainWindow::on_c4_toggled(bool checked)
{
    if(checked)
    {
        splineseries4->show();
...
void MainWindow::on_c5_toggled(bool checked)
{
    if(checked)
    {
        splineseries5->show();
...
void MainWindow::on_c6_toggled(bool checked)
{
    if(checked)
    {
        splineseries6->show();
...
void MainWindow::on_CutLink_clicked()
{
    //关闭串口
...
void MainWindow::on_TextReceive_toggled(bool checked)
{
    if(checked)
    {
